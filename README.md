...
| Figure | Script | Notes |
|--------|--------|-------|
| `fig_stability.pdf` | `python experiment_check_reconstruction_properties.py` | Gradient error vs matrix size (normal matrices) |
...
The accuracy figures are produced by `make_stability_figure()` and `make_reprojection_figure()` in `experiment_check_reconstruction_properties.py`. Running the script regenerates both PDFs in one pass.
