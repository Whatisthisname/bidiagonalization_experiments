...
As described in \ref{sec:bidiag_through_hessenberg}, we can use $\overline{\texttt{Bidiagonalize}}$ to (inefficiently) bidiagonalize any matrix $A$ with initial vector $v$. The detour through $\texttt{Hessenberg}$ let's us use the adjoint system of Hessenberg Factorization described in \ref{subsec:adjoint_hessenberg}. In this section we will inspect the adjoint system of $\overline{\texttt{Bidiagonalize}}$, and simplify it considerably by exploiting sparsity in $\bar{A}, \bar{v}, \bar{Q}, \bar{H}$ and $\bar{r}$.
...
We can now name the columns of $\Lambda$ 
$$\Lambda = \begin{bmatrix}\varphi_1 & \mathbf{0} & \varphi_2 & \mathbf{0} & \cdots &\varphi_k & \mathbf{0}\\ \mathbf{0} & \xi_1 & \mathbf{0} & \xi_2 &  \cdots  & \mathbf{0} & \xi_k \end{bmatrix} \in \mathbb{R}^{(N+M) \times K}$$
the entries of lower triangular $\Gamma$ as 
$$\Gamma = \begin{bmatrix} 
    \omega_{11} & 0 & 0 & 0 & 0 & 0 \\
    0 & \sigma_{11} & 0 & 0 & 0 & 0 \\
