...
The output gradients $\nabla_Q\rho$, $\nabla_H\rho$, $\nabla_r\rho$, $\nabla_c\rho$ will have know quantities, so we need only determine the adjoint variables $\Lambda$, $\Gamma$, $\lambda$, $\gamma$.
\\
...
\end{align}
Now, to solve for $\lambda_i$ and $\lambda$, we look again at \eqref{eq:adjoint_hessenberg_constraint1} and restrict again to column $i$:
\begin{align}
...
\end{align}
We now have all the equations necessary to solve for all adjoint variables. What remains is a solution order such that every equation's right hand side depends only on previously computed variables:
\\
