...
To compute $\texttt{Bidiagonalize}_K(A, v) = (L, R, B, r, c)$ for any $A \in \mathbb{R}^{N \times M}$ and $v \in \mathbb{R}^M$ using Hessenberg factorization, we construct the symmetric matrix $\bar{A}$ and augmented vector $\bar{v}$:
...
$\texttt{Hessenberg}_{2K}(\bar{A}, \bar{v}) = (\bar{Q}, \bar{H}, \bar{r}, c)$
which are directly related to $(L, R, B, r, c)$ through the following sparse equations:

$$\bar{Q} = \begin{bmatrix}\mathbf{0} & l_1 & \mathbf{0} & l_2 & \cdots & \mathbf{0} & l_k \\ r_1 & \mathbf{0} & r_2 & \mathbf{0} &  \cdots & r_k & \mathbf{0} \end{bmatrix} \in \mathbb{R}^{(N+M) \times 2K} \qquad \bar{r} :=\begin{pmatrix}\mathbf{0} \\ r \end{pmatrix} \in \mathbb{R}^{N+M}$$
...
\begin{align*}
    ...
    \\
    \implies & I_{\geq} \circ Q^\top \Lambda = I_{\geq} \circ \nabla_{H} \rho + I_{\geq} \circ I_{\ll} \circ \Sigma
    \\ 
    = &I_{\geq} \circ \nabla_{H} \rho
    \\
    \implies & Q^\top \lambda_K = \nabla_{H} \rho e_K
\end{align*}
This lets us express $\gamma$ in terms of the known quantities:
\begin{align}
    \eqref{eq:get_gamma_almost} \iff \gamma &= \nabla_{H} \rho e_K - Q^\top \nabla_r \rho \label{eq:get_gamma}
\end{align}
We now turn to \eqref{eq:adjoint_hessenberg_constraint1}.
\begin{align*}
    \eqref{eq:adjoint_hessenberg_constraint1} \overset{\eqref{eq:hessenberg_forward_constraint3}}{\implies} Q^\top \nabla_{Q} \rho + Q^\top A^\top \Lambda - Q^\top \Lambda H^\top + Q^\top \lambda (e_{1})^\top + (I_{\leq} \circ \Gamma) + (I_{\leq} \circ \Gamma)^\top = 0
    \\
    \implies 
\end{align*}
From primal constraints \eqref{eq:hessenberg_forward_constraint2} and \eqref{eq:hessenberg_forward_constraint3}, we have $c = 1/||v||$ and $v = q_1/c$, giving \eqref{eq:adjoint_hessenberg_constraint4} $\iff c\nabla_{c} \rho = q_1^\top \lambda$.


\subsection{Derivation of the Stable Adjoint System} \label{subsec:derivation_stable_adjoint_system}
As described in \ref{sec:bidiag_through_hessenberg}, we can use $\overline{\texttt{Bidiagonalize}}_K$ to (inefficiently) bidiagonalize any matrix $A$ with initial vector $v$. The detour through $\texttt{Hessenberg}$ let's us use the adjoint system of Hessenberg Factorization described in \ref{subsec:adjoint_hessenberg}. In this section we will inspect the adjoint system of $\overline{\texttt{Bidiagonalize}}_K$, and simplify it considerably by exploiting sparsity in $\bar{A}, \bar{v}, \bar{Q}, \bar{H}$ and $\bar{r}$.

The sparsity patterns of $\nabla_{\bar{Q}} \rho$, $\nabla_{\bar{H}} \rho$, and $\nabla_{\bar{r}} \rho$ match those of $\bar{Q}, \bar{H}, \bar{r}$, due to the linear nature of \texttt{extract}:
$$\nabla_{\bar{Q}} \rho = \begin{bmatrix}\mathbf{0} & \nabla_{l_1}\rho & \mathbf{0} & \nabla_{l_2}\rho & \cdots & \mathbf{0} & \nabla_{l_k}\rho \\ \nabla_{r_1}\rho & \mathbf{0} & \nabla_{r_2}\rho & \mathbf{0} &  \cdots & \nabla_{r_k}\rho & \mathbf{0} \end{bmatrix} \in \mathbb{R}^{(N+M) \times K} \qquad \nabla_{\bar{r}} \rho = \begin{pmatrix}\mathbf{0} \\ \nabla_{r} \rho \end{pmatrix} \in \mathbb{R}^{N+M}$$
$$\nabla_{\bar{H}} \rho = \begin{bmatrix}
    ...
    0 & 0 & 0 & 0 & 0 & 0  & 0 & 0\\
    \end{bmatrix} \in \mathbb{R}^{2K \times 2K}$$
We will look at the constraints \eqref{eq:adjoint_hessenberg_constraint1} through \eqref{eq:adjoint_hessenberg_constraint4} and determine further sparsity patterns in the adjoint variables $\Lambda, \Gamma, \lambda, \gamma$.


\subsubsection{Constraint \eqref{eq:adjoint_hessenberg_constraint1}}
\begin{align}
0 &= \nabla_{\bar{Q}} \rho + \bar{A}^\top \Lambda - \Lambda \bar{H}^\top + \lambda (e_{1})^\top 
   + \bar{Q}(I_{\leq} \circ \Gamma) + \bar{Q}(I_{\leq} \circ \Gamma)^\top + \bar{r} \gamma^\top
\end{align}

\subsubsection{Constraint \eqref{eq:adjoint_hessenberg_constraint2}}
\begin{align}
    0 &= \nabla_{\bar{H}} \rho - \bar{Q}^\top \Lambda + I_{\leq} \circ \Sigma
\end{align}


\subsubsection{Constraint \eqref{eq:adjoint_hessenberg_constraint3}}
\begin{align}
    0 &= \nabla_{\bar{r}} \rho - \Lambda e_{K} + \bar{Q} \gamma 
    \\
    &= \begin{pmatrix}\mathbf{0} \\ \nabla_{r} \rho \end{pmatrix} - \lambda_k + \bar{Q} \gamma
\end{align}

\subsubsection{Constraint \eqref{eq:adjoint_hessenberg_constraint4}}
\begin{align}
    0 &= \nabla_{c} \rho - \bar{v}^\top \lambda
    \\
    &= \nabla_{c} \rho - \begin{pmatrix}\mathbf{0} \\ v \end{pmatrix}^\top \lambda
\end{align}
\\
\subsubsection{Step 2: ???}
We will mark possibly nonzero entries with $*$, and constant zero entries with a $0$.\\
...
\begin{align}
    \eqref{eq:get_gamma} \implies \gamma &= \nabla_{\bar{H}} \rho e_{2K} - \bar{Q}^\top \nabla_{\bar{r}} \rho = \nabla_{\alpha_{K}} \rho e_{2K-1} - \bar{Q}^\top \begin{pmatrix}\mathbf{0} \\ \nabla_r\rho\end{pmatrix} 
    \\
    &= \begin{pmatrix}0 \\ 0 \\ 0 \\ 0 \\ \vdots \\ 0 \\ \nabla_{\alpha_K} \rho \\ 0\end{pmatrix}  - \begin{pmatrix}r_1^\top \\ \mathbf{0}^\top \\ r_2^\top \\ \mathbf{0}^\top \\ \vdots \\\mathbf{0}^\top\\ r_K^\top \\ \mathbf{0}^\top\end{pmatrix} \nabla_{r} \rho = \begin{pmatrix}* \\ 0 \\ * \\ 0 \\ \vdots \\ 0 \\ * \\ 0\end{pmatrix}
\end{align}
We plug this pattern into \eqref{eq:get_last_lambda}, which shows that $\lambda_2K$ has the same sparsity pattern as $\bar{v}$:
\begin{align}
    \eqref{eq:get_last_lambda} \implies \lambda_2K &= \nabla_{\bar{r}} \rho + \bar{Q} \gamma
    = \begin{pmatrix}\mathbf{0} \\ \nabla_r\rho\end{pmatrix}  + \bar{Q} \begin{pmatrix}* \\ 0 \\ * \\ 0 \\ \vdots \\ 0\\ * \\ 0\end{pmatrix}
    = \begin{pmatrix}0 \\ 0 \\ \vdots \\ 0 \\ * \\ * \\ \vdots \\ *\end{pmatrix} + \begin{pmatrix}0 \\ 0 \\ \vdots \\ 0 \\ * \\ * \\ \vdots \\ *\end{pmatrix}
